...
#include "lidar_localization/models/scan_adjust_imu/distortion_adjust_imu.hpp"


namespace lidar_localization{
    void DistortionAdjust_Imu::SetMotionInfo_imu(float scan_period, IMUData velocity_data) {
    ...
    bool DistortionAdjust_Imu::AdjustClound(CloudData::CLOUD_PTR &input_cloud_ptr, CloudData::CLOUD_PTR &output_cloud_ptr) {
        ...
        for(size_t point_index = 1;point_index < origin_cloud_ptr->points.size();++point_index){
            ...
            float real_time = fabs(orientation) / orientation * scan_period - scan_period / 2.0;
            ...
            Eigen::Matrix3f curren_matrix = UpdateMatrix(real_time);
            Eigen::Vector3f rotated_point = curren_matrix * origin_point;
    ...
    Eigen::Matrix3f DistortionAdjust_Imu::UpdateMatrix(float real_time) {
