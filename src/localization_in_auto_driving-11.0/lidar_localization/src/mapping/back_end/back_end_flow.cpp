...
BackEndFlow::BackEndFlow(ros::NodeHandle& nh) {
    ...
    transformed_odom_pub_ptr_ = std::make_shared<OdometryPublisher>(nh, "/transformed_odom", "/map", "/velodyne", 100);
...
bool BackEndFlow::UpdateBackEnd() {
    ...
    float laser_odom_x = current_laser_odom_data_.pose(0,3);
    float laser_odom_y = current_laser_odom_data_.pose(1,3);
    float laser_odom_z = current_laser_odom_data_.pose(2,3);
    Eigen::Vector3d eulerAngle1 = current_laser_odom_data_.pose.block<3,3>(0,0).eulerAngles(2,1,0).cast<double>(); // ZYX顺序，yaw,pitch,roll
//    std::cout << "laser_odom laser_odom_x laser_odom_y laser_odom_z" << laser_odom_x << laser_odom_y << laser_odom_z;
//    std::cout << "laser_odom roll pitch yaw = " << eulerAngle1[2] << " " << eulerAngle1[1]
