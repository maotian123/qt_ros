...
FrontEndFlow::FrontEndFlow(ros::NodeHandle& nh) {
    ...
    laser_odom_pub_ptr_ = std::make_shared<OdometryPublisher>(nh, "/laser_odom", "/map", "velodyne", 100);
...
bool FrontEndFlow::ReadData() {
    static bool start_fron = false;
    if(!start_fron){
        std::cout<<"Front begin read Data"<<std::endl;
        start_fron = true;
    }
